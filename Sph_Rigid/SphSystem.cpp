...
*/
	   {
		   double bcdx = dx/2, bcdz = dz/2;
		   int bcnpx = npx*2 + 1, bcnpz = npz*2 - 1;
		   int bcnp = bc_count; 
	  for(int i=0; i<bcnp; i++)
	   {
		   if(i < bcnpx) //bottom
		   {
			bcparticles->position.z = 0.0 - bcdz;
	   	   	bcparticles->position.x = (bcparticles->id -1) * bcdx - bcdx;
		   } else if( i < bcnpx + bcnpz) //left side
		   {
			   bcparticles->position.x = 0.001+ bcdx;
			   bcparticles->position.z = (bcparticles->id -1 - bcnpx)* bcdz;
		   }else if( i< 2*bcnpx + bcnpz) //top
		   { 
			   bcparticles->position.z = 0.001 + bcdz ;
			   bcparticles->position.x = (3*bcnp/4 - bcparticles->id) * bcdx;
		   } else 
		   { 	
			   bcparticles->position.x = 0.000 - bcdx ;
			   bcparticles->position.z = (bcnp - bcparticles->id) * bcdz;
		  }
		    bcparticles++;		 
	}
